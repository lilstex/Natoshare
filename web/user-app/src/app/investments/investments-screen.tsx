    ...
    <div>
      ...
      <main className="mx-auto max-w-3xl px-4 pb-16">
        ...
        <div className="mt-3 flex flex-col gap-2">
          ...
          {logs.map((log) => (
            <div key={log.id} className="flex items-center justify-between rounded-xl border border-border bg-surface px-4 py-2.5">
              <div>
                <p className="text-sm text-text">{log.platform}</p>
                <p className="text-xs text-muted">{log.investedOn}</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <button type="button" onClick={() => handleDelete(log.id)} className="text-xs font-medium text-muted hover:text-danger">
                  Remove
                </button>
              </div>
            </div>
